  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer() {
        let menuoptions = [
          {
            value: "Mensajes de Bienvenida en Canal",
            description: `Gestiona los mensajes de bienvenida en 1 CANAL`,
            emoji: "895066899619119105" //
          },
          {
            value: "Mensajes de Bienvenida en Canal 2",
            description: `Establece un mensaje normal para un segundo canal (sin Embed)`,
            emoji: "895066899619119105" //
          },
          {
            value: "Mensajes directos de bienvenida",
            description: `Gestiona los mensajes de bienvenida por MD`,
            emoji: "😬"
          },
          {
            value: "Roles de bienvenida (al unirse)",
            description: `Gestiona los Roles de Bienvenida. ¡Añade/elimina!`,
            emoji: "895066900105674822"
          },
          {
            value: "Sistema Captcha (Seguridad)",
            description: `${client.settings.get(message.guild.id, "welcome.captcha") ? "❌ Desactivar el sistema de seguridad Captcha" : "✅ Activar el sistema de seguridad Captcha"}`,
            emoji: "866089515993792522"
          },
          {
            value: `Prueba bienvenida`,
        ...
        let Selection = new MessageSelectMenu()
          ...
          .setPlaceholder('Haz Clic para Configurar el Sistema de Bienvenidas')
          .addOptions(
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor({ name: "Sistema de Bienvenidas", url: "https://discord.gg/C4yXhc8u3t", iconURL: "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png" })
          //.setAuthor('Sistema de Bienvenidas', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.gg/C4yXhc8u3t')
        ...
        collector.on('collect', menu => {
          ...
          else menu?.reply({ content: `<a:no:1015072569612828754> ¡No estás permitido para hacer eso! Sólo: <@${cmduser.id}>`, ephemeral: true });
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          menumsg.edit({ embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes:1029225889076092958> **Seleccionado: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "<a:no:1015072569612828754> **NADA SELECCIONADO - CANCELADO**"}` })
        });
      }

      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {
          case "Mensajes de Bienvenida en Canal": {

            second_layer()
            async function second_layer() {
              let menuoptions = [
                {
                  value: `${client.settings.get(message.guild.id, "welcome.channel") == "nochannel" ? "Establecer canal" : "Sobreescribir canal"}`,
                  description: `${client.settings.get(message.guild.id, "welcome.channel") == "nochannel" ? "Establece un canal en el que deben aparecer los mensajes de bienvenida" : "Sobrescribir el canal actual con uno nuevo"}`,
                  emoji: "895066899619119105" //
                },
                {
                  value: "Desactivar Sistema de Bienvenidas",
                  description: `Desactivar los mensajes de bienvenida`,
                  emoji: "❌"
                },
                {
                ...
                {
                  value: `${client.settings.get(message.guild.id, "welcome.invite") ? "Desactivar información de invitaciones" : "Activar información de invitaciones"}`,
                  description: `${client.settings.get(message.guild.id, "welcome.invite") ? "No se mostrara la información de la invitación" : "Se mostrara información de la invitación"}`,
                  emoji: "877653386747605032"
                },
                {
                  value: "Cancelar",
              ...
              collector.on('collect', menu => {
                if (menu?.user.id === cmduser.id) {
                  collector.stop();
                  let menuoptiondata = menuoptions.find(v => v.value == menu?.values[0])
                  if (menu?.values[0] == "Cancelar") return menu?.reply(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable3"]))
                  menu?.deferUpdate();
                  let SetupNumber = menu?.values[0].split(" ")[0]
                  handle_the_picks_2(menu?.values[0], SetupNumber, menuoptiondata)
                }
                else menu?.reply({ content: `<a:no:1015072569612828754> ¡No estás permitido para hacer eso! Sólo: <@${cmduser.id}> puede`, ephemeral: true });
              });
              //Once the Collections ended edit the menu message
              collector.on('end', collected => {
                menumsg.edit({ embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes:1029225889076092958> **Seleccionado: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "<a:no:1015072569612828754> **NADA SELECCIONADO - CANCELADO**"}` })
            ...
            async function handle_the_picks_2(optionhandletype, SetupNumber, menuoptiondata) {
              switch (optionhandletype) {
                case `${client.settings.get(message.guild.id, "welcome.channel") == "nochannel" ? "Establecer canal" : "Sobreescribir canal"}`: {
                      ...
                      if (channel) {
                        ...
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            ...
                            .setColor(es.color)
                            .setDescription(`Si alguien se une a este servidor, se enviará un mensaje a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "No definido aún"}!\nEdita el mensaje con: \`${prefix}setup-welcome\``.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } else {
                        return message.reply("No has hecho mencionado un canal válido")
                      }
                    })
                    .catch(e => {
                ...
                case `Desactivar Sistema de Bienvenidas`: {
                ...
                case `Gestionar la imagen`: {
                  ...
                  async function third_layer() {
                    let menuoptions = [
                      {
                        value: "Desactiva la imagen",
                        description: `No enviare imagenes`,
                        emoji: "❌"
                      },
                      {
                        value: "Activar la imagen automática",
                      ...
                      },
                      {
                        ...
                        description: `Define el fondo de la AUTO IMAGEN`,
                        emoji: "👍"
                      },
                      {
                        value: "Eliminar el Fondo de la imagen",
                        description: `Restablece el fondo de la imagen automática a la predeterminada`,
                        emoji: "🗑"
                      },
                      {
                      ...
                      {
                        value: `${client.settings.get(message.guild.id, "welcome.frame") ? "Desactivar" : "Activar"} Marco`,
                        description: `${client.settings.get(message.guild.id, "welcome.frame") ? "No mostraré más el Marco" : "Me permite mostrar un marco de color para resaltar"}`,
                        emoji: "✏️"
                      },
                      {
                        value: `${client.settings.get(message.guild.id, "welcome.discriminator") ? "Desactivar" : "Activar"} Etiqueta de usuario`,
                        description: `${client.settings.get(message.guild.id, "welcome.discriminator") ? "No mostraré más la etiqueta del usuario" : "Me permite mostrar una etiqueta de usuario de color (#1234)"}`,
                        emoji: "🔢"
                      },
                      {
                        value: `${client.settings.get(message.guild.id, "welcome.membercount") ? "Desactivar" : "Activar"} Cuenta de Miembros`,
                        description: `${client.settings.get(message.guild.id, "welcome.membercount") ? "No mostraré más la cuenta de miembros" : "Me permite mostrar una cuenta de miembros coloreada del Servidor"}`,
                        emoji: "📈"
                      },
                      {
                        value: `${client.settings.get(message.guild.id, "welcome.servername") ? "Desactivar" : "Activar"} Nombre del Servidor`,
                        description: `${client.settings.get(message.guild.id, "welcome.servername") ? "Ya no mostraré el nombre del servidor" : "Me permite mostrar un nombre de servidor coloreado"}`,
                        emoji: "🗒"
                      },
                      {
                        value: `${client.settings.get(message.guild.id, "welcome.pb") ? "Desactivar" : "Activar"} Avatar de Usuario`,
                        description: `${client.settings.get(message.guild.id, "welcome.pb") ? "Ya no mostraré el avatar de usuario" : "Me permite mostrar el avatar de usuario"}`,
                        emoji: "💯"
                      },
                      {
                        value: "Color de Marco",
                        description: `Cambia el Color del marco`,
                        emoji: "⬜"
                      },
                      {
                        value: "Cancelar",
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata) {
                    switch (optionhandletype) {
                      case `Desactiva la imagen`: {
                        client.settings.set(message.guild.id, false, "welcome.image")
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            .setTitle(eval(client.la[ls]["cmds"]["setup"]["setup-welcome"]["variable18"]))
                            .setColor(es.color)
                            .setDescription(`Si alguien se une a este servidor, se enviará un mensaje **sin imagen** a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO SE HA DEFINIDO UN CANAL"}`.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } break;
                      case `Activar la imagen automática`: {
                        client.settings.set(message.guild.id, true, "welcome.image")
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            .setTitle(eval(client.la[ls]["cmds"]["setup"]["setup-welcome"]["variable21"]))
                            .setColor(es.color)
                            .setDescription(`Usare ${client.settings.get(message.guild.id, "welcome.custom") === "no" ? "una imagen generada automáticamente con datos del usuario" : "Tu imagen definida y personalizada"}\n\nSi alguien se une a este servidor, se enviará un mensaje **con una imagen** a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO SE HA DEFINIDO UN CANAL"}`.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } break;
                              ...
                              if (collected.first().attachments.every(attachIsImage)) {
                                ...
                                return message.reply({
                                  embeds: [new Discord.MessageEmbed()
                                    ...
                                    .setColor(es.color)
                                    .setDescription(`Voy a utilizar ${client.settings.get(message.guild.id, "welcome.custom") === "no" ? "una imagen generada automáticamente con datos del usuario" : "Tu imagen definida y personalizada"}\n\nSi alguien se une a este servidor, se enviará un mensaje **con una imagen** a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO SE HA DEFINIDO UN CANAL"}`.substring(0, 2048))
                                    .setFooter(client.getFooter(es))
                                  ]
                                });
                              } else {
                      ...
                      case `Eliminar el Fondo de la imagen`: {
                        client.settings.set(message.guild.id, true, "welcome.image")
                        ...
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            ...
                            .setColor(es.color)
                            .setDescription(`If Someone joins this Server, a message **with an image** will be sent into ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO SE HA DEFINIDO UN CANAL"}`.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } break;
                      ...
                      case `${client.settings.get(message.guild.id, "welcome.frame") ? "Desactivar" : "Activar"} Marco`: {
                        client.settings.set(message.guild.id, "no", "welcome.custom")
                        ...
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            ...
                            .setColor(es.color)
                            .setDescription(`Si alguien se une a este servidor, se enviará un mensaje **con una imagen automatizada** a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO SE HA DEFINIDO UN CANAL"}`.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } break;
                      case `${client.settings.get(message.guild.id, "welcome.discriminator") ? "Desactivar" : "Activar"} Etiqueta de usuario`: {
                        client.settings.set(message.guild.id, "no", "welcome.custom")
                      ...
                      case `${client.settings.get(message.guild.id, "welcome.membercount") ? "Desactivar" : "Activar"} Cuenta de Miembros`: {
                        client.settings.set(message.guild.id, "no", "welcome.custom")
                      ...
                      case `${client.settings.get(message.guild.id, "welcome.servername") ? "Desactivar" : "Activar"} Nombre del Servidor`: {
                        client.settings.set(message.guild.id, "no", "welcome.custom")
                      ...
                      case `${client.settings.get(message.guild.id, "welcome.pb") ? "Desactivar" : "Activar"} Avatar de Usuario`: {
                        client.settings.set(message.guild.id, "no", "welcome.custom")
                      ...
                      case `Color de Marco`: {
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({ embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:1029225889076092958> **Seleccionado the \`${collected.first().customId}\` Color**` : "<a:no:1015072569612828754> **NADA SELECCIONADO - CANCELADO**"}` })
                        });
                        //Menu Collections
                        collector.on('collect', async button => {
                          if (button?.user.id === cmduser.id) {
                            ...
                            button?.reply(":x: **¡Sólo el que uso el comando puede reaccionar!**")
                          }
                        })
                      } break;
                    }
                ...
                case `Editar el Mensaje`: {
                  tempmsg = await message.reply({
                    embeds: [new Discord.MessageEmbed()
                      ...
                      .setColor(es.color)
                      .setDescription(`\`{user}\` ... será sustituido por la mención al usuario (por ejemplo ${cmduser})\n\`{username}\` ... sera sustituido por el nombre de usuario (Por ejemplo ${cmduser.user.username})\n\`{usertag}\` ... sera sustituido por el tag del usuario (Por ejemplo ${cmduser.user.tag})\n\n**Envia el mensaje!**`)
                      .setFooter(client.getFooter(es))]
                  })
                  await tempmsg.channel.awaitMessages({
                    filter: m => m.author.id === cmduser.id,
                    ...
                    .then(collected => {
                      ...
                      return message.reply({
                        embeds: [new Discord.MessageEmbed()
                          ...
                          .setColor(es.color)
                          .setDescription(`Si alguien se une a este servidor, este mensaje se enviará a  ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "NO CHANNEL YET"}!\n\n${message.content.replace("{user}", `${cmduser.user}`).replace("{username}", `${cmduser.user.username}`).replace("{usertag}", `${cmduser.user.tag}`)}`.substring(0, 2048))
                          .setFooter(client.getFooter(es))
                        ]
                      });
                    })
                ...
                case `${client.settings.get(message.guild.id, "welcome.invite") ? "Disable InviteInformation" : "Enable Invite Information"}`: {
                  ...
                  return message.reply({
                    embeds: [new Discord.MessageEmbed()
                      ...
                      .setColor(es.color)
                      .setDescription(`Si alguien se une a este servidor, se enviará un mensaje con información de la invitación a ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.channel")) : "No definido aún"}!\nEdita el Mensaje con: \`${prefix}setup-welcome\``.substring(0, 2048))
                      .setFooter(client.getFooter(es))
                    ]
                  });
                } break;
          ...
          case "Mensajes de Bienvenida en Canal 2": {
            second_layer()
            async function second_layer() {
              let menuoptions = [
                {
                  value: `${client.settings.get(message.guild.id, "welcome.secondchannel") == "nochannel" ? "Establecer Canal" : "Sobreescribir canal"}`,
                  description: `${client.settings.get(message.guild.id, "welcome.secondchannel") == "nochannel" ? "Establece un canal en el que deben aparecer los mensajes de bienvenida" : "Sobrescribir el canal actual con uno nuevo"}`,
                  emoji: "895066899619119105" //
                },
                {
                  value: "Desactivar Bienvenida 2",
                  description: `Desactiva el segundo mensaje de bienvenida`,
                  emoji: "❌"
                },
                {
                  value: "Editar el Mensaje",
                  description: `Edita el segundo Mensaje de Bienvenida`,
                  emoji: "877653386747605032"
                },
                {
                  value: "Cancelar",
                      ...
                      if (channel) {
                        ...
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            .setTitle(eval(client.la[ls]["cmds"]["setup"]["setup-welcome"]["variable9"]))
                            .setColor(es.color)
                            .setDescription(`Si alguien se une a este servidor, se enviará un mensaje a  ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.secondchannel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.secondchannel")) : "No definido aún"}!\nEdita el Mensaje con: \`${prefix}setup-welcome\``.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } else {
                    ...
                    .then(collected => {
                      ...
                      return message.reply({
                        embeds: [new Discord.MessageEmbed()
                          .setTitle(eval(client.la[ls]["cmds"]["setup"]["setup-welcome"]["variable66"]))
                          .setColor(es.color)
                          .setDescription(`Si alguien se une a este servidor, este mensaje se enviará a  ${message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.secondchannel")) ? message.guild.channels.cache.get(client.settings.get(message.guild.id, "welcome.secondchannel")) : "NO CHANNEL YET"}!\n\n${message.content.replace("{user}", `${cmduser.user}`).replace("{username}", `${cmduser.user.username}`).replace("{usertag}", `${cmduser.user.tag}`)}`.substring(0, 2048))
                          .setFooter(client.getFooter(es))
                        ]
                      });
                    })
          ...
          case "Mensajes directos de bienvenida": {
            ...
            async function second_layer() {
              let menuoptions = [
                {
                  value: `${!client.settings.get(message.guild.id, "welcome.dm") ? "Activar bienvenida por MD" : "Desactivar bienvenida por MD"}`,
                  description: `${!client.settings.get(message.guild.id, "welcome.dm") ? "Envía mensajes de bienvenida directamente a los usuarios" : "No envía mensajes de bienvenida directamente a los usuarios"}`,
                ...
                {
                  value: `${client.settings.get(message.guild.id, "welcome.invite") ? "Desactivar la información de las invitaciones" : "Activar la información de las invitaciones"}`,
                  description: `${client.settings.get(message.guild.id, "welcome.invite") ? "No se muestra la información de la invitación" : "Se muestra la información de la invitación"}`,
                  emoji: "877653386747605032"
                },
                {
                  value: "Cancelar",
              ...
              let MenuEmbed = new MessageEmbed()
                .setColor(es.color)
                .setAuthor('DM - Sistema de Bienvenidas', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.gg/C4yXhc8u3t')
                .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
              //send the menu msg
              let menumsg = await message.reply({ embeds: [MenuEmbed], components: [new MessageActionRow().addComponents(Selection)] })
              //Create the collector
            ...
            async function handle_the_picks_2(optionhandletype, SetupNumber, menuoptiondata) {
              switch (optionhandletype) {
                case `${!client.settings.get(message.guild.id, "welcome.dm") ? "Activar bienvenida por MD" : "Desactivar bienvenida por MD"}`: {
                              ...
                              if (collected.first().attachments.every(attachIsImage)) {
                                ...
                                return message.reply({
                                  embeds: [new Discord.MessageEmbed()
                                    ...
                                    .setColor(es.color)
                                    .setDescription(`Estare usando ${client.settings.get(message.guild.id, "welcome.customdm") === "no" ? "Una imagen autogenerada con datos del usuario" : "Tu imagen definida, personalizada"}`.substring(0, 2048))
                                    .setFooter(client.getFooter(es))
                                  ]
                                });
                              } else {
          ...
          case "Roles de bienvenida (al unirse)": {
            ...
            async function second_layer() {
              let menuoptions = [
                {
                  value: "Añadir Rol",
                  description: `Añade un Rol de Bienvenida`,
                  emoji: "✅"
                },
                {
                  value: "Remover Rol",
                  description: `Remover un Rol de bienvenida`,
                  emoji: "🗑️"
                },
                {
                  value: "Mostrar Roles",
                  description: `Mostrar todos los roles de bienvenida`,
                  emoji: "📑"
                },
                {
                  value: "Cancelar",
                      ...
                      if (role) {
                        ...
                        return message.reply({
                          embeds: [new Discord.MessageEmbed()
                            ...
                            .setColor(es.color)
                            .setDescription(`Todos los que se unan obtendrán estos Roles ahora:\n<@&${client.settings.get(message.guild.id, "welcome.roles").join(">\n<@&")}>`.substring(0, 2048))
                            .setFooter(client.getFooter(es))
                          ]
                        });
                      } else {
                        return message.reply("No has mencionado un Rol valido")
                      }
                    })
                    .catch(e => {
                      console.log(e.stack ? String(e.stack).grey : String(e).grey)
          ...
          case "Sistema Captcha (Seguridad)": {
            ...
            return message.reply({
              embeds: [new Discord.MessageEmbed()
                ...
                .setColor(es.color)
                .setDescription(`${client.settings.get(message.guild.id, "welcome.captcha") ? "Pediré a los nuevos miembros que se verifiquen, luego les enviaré mensajes de bienvenida / les añadiré los roles si lo consiguen, + les echaré si no lo consiguen" : "¡No pediré a los nuevos miembros que se verifiquen!"}`.substring(0, 2048))
                .setFooter(client.getFooter(es))
              ]
            });
          } break;
          case `Prueba bienvenida`: {
            var { member } = message;
            let welcome = client.settings.get(member.guild.id, "welcome");
            let invitemessage = `Invitado/a por ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:like:998118554094272512> **X Invitaciones**\n[<:joined:998120174567837716> X Entradas | <:leave:998120508111474760> X Salidas | <a:no:1015072569612828754>> X Falsas]`
            if (welcome) {
              let themessage = String(welcome.secondmsg);
              if (!themessage || themessage.length == 0) themessage = ":wave: {user} **Bienvenido/a a nuestro Servidor** :v:";
              ...
              if (message.channel.permissionsFor(message.channel.guild.me).has(Discord.Permissions.FLAGS.SEND_MESSAGES)) {
                message.channel.send({ content: `**CANAL 2 MENSAJE en ${welcome.secondchannel != "nochannel" ? `<#${welcome.secondchannel}>` : ` \`NO HAY CANAL CONFIGURADO\``}:**\n\n${themessage}`.substring(0, 2000) }).catch(() => { });
              }
            }
            ...
            async function msg_withoutimg(member) {
              ...
              if (channel.permissionsFor(channel.guild.me).has(Discord.Permissions.FLAGS.SEND_MESSAGES)) {
                if (channel.permissionsFor(channel.guild.me).has(Discord.Permissions.FLAGS.EMBED_LINKS)) {
                  channel.send({
                    content: `**CANAL DE BIENVENIDA en ${welcome.channel != "nochannel" ? `<#${welcome.channel}>` : ` \`NO HAY CANAL CONFIGURADO\``}:**\n\n<@${member.user.id}>`,
                    embeds: [welcomeembed]
                  }).catch(() => { });
                } else {
                  channel.send({
            ...
            async function dm_msg_withoutimg(member) {
              ...
              //send the welcome embed to there
              channel.send({
                content: `**BIENVENIDA EN MENSAJE DIRECTO:**\n\n<@${member.user.id}>`,
                embeds: [welcomeembed]
              }).catch(() => { });
            }
