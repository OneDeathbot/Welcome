...
module.exports = client => {
  ...
  client.on("guildMemberAdd", async mem => {
    ...
    async function welcome(member) {
      ...
      if (invite && inviter) {
        ...
        invitemessage = `Invitado por ${inviter.tag ? `**${inviter.tag}**` : `<@${inviter.id}>`}\n<:like:998118554094272512> **${realinvites} Invitacion${realinvites == 1 ? "" : "es"}**\n[<:joined:998120174567837716> ${invites} Entradas | <:leave:998120508111474760> ${leaves} Salidas | <a:no:1015072569612828754> ${fake} Falsas]`;
      } 
      else if(invite){
        invitemessage = `Invitado por un **Mimebro Desconocido**`
      }
      else if (vanity) {
        try{
          let res = await member.guild.fetchVanityData().catch(() => {});
          if(res){
            invitemessage = `Invitado por una **[Vanity URL](https://discord.gg/${res.code})** with \`${res.uses} Uses\``
          } else {
            invitemessage = `Invitado por una **Vanity Link!**`;
          }
        }catch (e){
          console.log(e.stack ? String(e.stack).grey : String(e).grey)
          invitemessage = `Invitado por a **Vanity Link!**`;
        }
      } else if (perm) {
        //get the new memberdata
        invitemessage = `Necesito el permiso de **MANEJAR SERVIDORES** para poder mostrar las invitaciones!`;
      } else {
        invitemessage = "\u200b"
      }
       ...
       const captcha = new Captcha();
        ...
        member.send({content: `**${member.guild.name}** has a Captcha Security Option enabled!\n> Solve it first, by typing the WHOLE LETTERS of the IMAGE!`, embeds: [captchaembed], files: [attachment]}).then(msg => {
          ...
          member.guild.channels.create(`verify-${member.user.username}`.substring(0, 32), {
            ...
            member.kick().catch(() => {});
            member.guild.fetchOwner().then(owner => {
              owner.send(`<a:aviso:1012887565268947015> **No puedo crear Canales para el Usuario Captcha, necesito permisos para ello lo antes posible!**`).catch(() => {});
            }).catch(() => {});
          })
        })
      } else {
    ...
    async function message(member) {
      ...
      if(welcome && welcome.secondchannel !== "nochannel"){
        ...
        if(!themessage || themessage.length == 0) themessage = ":wave: {user} **Bienvenid@ a nuestro Servidor!** :v:";
  ...
  client.on("guildMemberAdd", async member => {
    ...
    client.settings.ensure(member.guild.id, {
      antinewaccount: {
        ...
        extra_message: "No vuelvas a unirte, a menos que cumplas los requisitos!"
      } 
    });
    ...
    if(action == "ban") {
      await member.send({
        embeds: [
          ...
          .setTitle(`Haz sido baneado de __${member.guild.name}__`)
          .setThumbnail(member.guild.iconURL({dynamic: true}))
          .setFooter({text: `${member.guild.name} | ${member.guild.id}`, iconURL: `${member.guild.iconURL({dynamic: true})}`})
          .setDescription(`Esto se debe a que tu cuenta fue creada ${duration(Date.now() - createdAccount).map(a => `\`${a}\``).join(", ")} ago, and the minimum Amount of Account-Age should be: ${duration(newaccount_delay).map(a => `\`${a}\``).join(", ")}`)
          .addField(`**Mensaje del Servidor:**`, `${extramessage && extramessage.length > 1 ? extramessage : "Mensaje no añadido"}`.substring(0, 1024))
        ]
      }).catch(() => {});
      member.ban({reason: `Detección de multicuenta | Cuenta creada ${duration(Date.now() - createdAccount).join(", ")} ago`})
    } else {
      await member.send({
        embeds: [
          new Discord.MessageEmbed()
          .setTitle(`Haz sido expulsado de __${member.guild.name}__`)
          .setThumbnail(member.guild.iconURL({dynamic: true}))
          .setFooter({text: `${member.guild.name} | ${member.guild.id}`, iconURL: `${member.guild.iconURL({dynamic: true})}`})
          .setDescription(`Esto se debe a que tu cuenta fue creada ${duration(Date.now() - createdAccount).map(a => `\`${a}\``).join(", ")} ago, and the minimum Amount of Account-Age should be: ${duration(newaccount_delay).map(a => `\`${a}\``).join(", ")}`)
          .addField(`**Mensaje del Servidor:**`, `${extramessage && extramessage.length > 1 ? extramessage : "Mensaje no añadido"}`.substring(0, 1024))
      ...
      member.kick({reason: `Detección de multicuenta | Cuenta creada ${duration(Date.now() - createdAccount).join(", ")} ago`})
    }
  })
  ...
  client.on("guildMemberAdd", async member => {
    ...
    if(!member.user.avatarURL() && joinlist.noavatar.filter(d => d.data == "enable").length > 0) {
      const reason = '`El usuario no tiene avatar (joinlist)`'
    ...
    if(joinlist.server_in_common.map(d => d.data).length > 0) {
      ...
      const reason = '`Estas en el Servidor ${inthere.name} (joinlist)`'
    ...
    if(joinlist.server_not_in_common.map(d => d.data).length > 0) {
      ...
      const reason = '`No estas en el servidor ${notInthere.name} (joinlist)`'
      ...
      return new Promise(async (resolve, reject) => {
        if(datas.length > 0) {
          for(const data of datas) {
            if(data.action == "kick") {
              if(member.kickable) {
                await member.send(`Has sido expulsado de \`${member.guild.name}\` porque:\n> ${eval(reason)}`).catch(() => {});
